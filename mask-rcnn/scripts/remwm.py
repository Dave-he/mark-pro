...
@click.option("--iopaint-model-dir", type=click.Path(), default=None, help="iopaint模型目录，可选")
@click.option("--iopaint-model-name", type=str, default="lama", help="iopaint模型名，默认lama")
def main(input_path: str, output_path: str, overwrite: bool, transparent: bool, max_bbox_percent: float, force_format: str, iopaint_model_dir: str, iopaint_model_name: str):
    ...
    if not transparent:
        model_manager = ModelManager(name=iopaint_model_name, device=device, model_dir=iopaint_model_dir)
        logger.info(f"LaMa/iopaint模型已加载，name={iopaint_model_name}, dir={iopaint_model_dir}")
    ...
    def handle_one(image_path: Path, output_path: Path):
        ...
        # 新增：保存mask图像
        mask_save_path = output_path.with_name(output_path.stem + "_mask.png")
        mask_image.save(mask_save_path)
        logger.info(f"Florence-2 mask saved: {mask_save_path}")
...
"""
python remwm.py input out1 --iopaint-model-dir .cache --iopaint-model-name lama
"""